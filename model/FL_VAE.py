...
class FL_CVAE_Medical(AbstractAutoEncoder):
    ...
    def __init__(self, args, d=128, z=32, device=None, with_classifier=True, **kwargs):
        ...
        # Encoder architecture optimized for sparse medical tabular data
        self.encoder = nn.Sequential(
        ...
        # Decoder architecture optimized for sparse reconstruction
        self.decoder = nn.Sequential(
            nn.Linear(self.hidden_dim, self.hidden_dim),
            nn.BatchNorm1d(self.hidden_dim),
            nn.ReLU(inplace=True),
            nn.Dropout(0.4),  # High dropout to encourage sparsity in reconstruction
