"""
Jupyter/Colab-friendly visualization of performance-sensitive (rx) features.
Run this after Phase 1 VAE training completion to see hospital clustering.
No argparse - use configuration variables instead.
"""
...
DATA_DIR = "rx_features_visualization"
METHOD = "tsne"  # 'tsne' or 'umap'
OUTPUT_DIR = "rx_visualization"
...
def load_rx_features(data_dir=DATA_DIR):
    ...
    for file in files:
        ...
        # Handle hospital_id - use client_id as fallback if hospital_id is None
        hospital_id_raw = data['hospital_id']
        if hospital_id_raw is None or (isinstance(hospital_id_raw, str) and hospital_id_raw.lower() == 'none'):
            hospital_id = client_id  # Use client_id as fallback
        else:
...
def combine_features(client_data, max_samples_per_client=MAX_SAMPLES_PER_CLIENT):
...
def compute_embedding(features, method=METHOD):
...
def plot_comparison(combined_data, method=METHOD, output_dir=OUTPUT_DIR):
...
def run_visualization():
    ...
    # Load data
    client_data = load_rx_features(DATA_DIR)
    if client_data is None:
        print("No data loaded. Make sure to run training with extract_features=true first.")
        return None
    
    # Combine features
    combined_data = combine_features(client_data, MAX_SAMPLES_PER_CLIENT)
    print(f"Combined data: {len(combined_data['targets'])} total samples")
    
    # Create visualization
    results = plot_comparison(combined_data, METHOD, OUTPUT_DIR)
...
# For Jupyter/Colab usage - just run this cell
